(() => {
  ...
  const css = `
    .pbt { position:fixed; z-index:2147483640; inset:0; overflow:auto; color:#f7f5ff; background:radial-gradient(ellipse 70% 45% at 12% -15%,#532bc04f,transparent 70%),radial-gradient(ellipse 55% 45% at 105% 90%,#0dd6df24,transparent 72%),#050617; font:14px Inter,Arial,sans-serif; }
    .pbt * { box-sizing:border-box; } .pbt button { font:inherit; } .pbt-shell { width:min(1180px,calc(100% - 48px)); margin:auto; }
    ...
    .pbt-detail { animation:pbt-enter .32s ease-out; } .pbt-hero { position:relative; display:grid; grid-template-columns:1.1fr .9fr; gap:22px; min-height:262px; margin-top:20px; padding:30px; overflow:hidden; border:1px solid #9f55ff9e; border-radius:22px; background:linear-gradient(115deg,#12175ce8,#1c0b4fdf 56%,#061d3ddf); box-shadow:0 0 42px #7e3eff26; } .pbt-hero:before { content:''; position:absolute; width:460px; height:460px; right:-160px; top:-230px; border:1px solid #915dff6e; border-radius:50%; box-shadow:0 0 0 44px #8954ff12,0 0 0 92px #29e0ea0b; animation:pbt-orbit 11s linear infinite; } .pbt-kicker { color:#26ebd4; font-size:11px; font-weight:800; letter-spacing:.12em; text-transform:uppercase; } .pbt-hero h1 { position:relative; margin:9px 0 10px; font-size:35px; } .pbt-hero-copy { max-width:570px; } .pbt-hero-actions { display:flex; flex-wrap:wrap; gap:10px; margin-top:20px; } .pbt-main-action { border-color:#f170ff; background:linear-gradient(90deg,#624bff,#e33be0); box-shadow:0 0 24px #bd43ef6b; } .pbt-main-action:disabled { opacity:.45; cursor:not-allowed; box-shadow:none; }
    ...
    @media(max-width:800px){ .pbt-shell{width:calc(100% - 32px)}.pbt-header{height:66px}.pbt-status,.pbt-close{display:none}.pbt-main{padding:20px 0 98px}.pbt-title-row{display:block}.pbt h1{font-size:31px}.pbt-grid{grid-template-columns:1fr;margin-top:18px}.pbt-hero{grid-template-columns:1fr;padding:21px;min-height:0}.pbt-hero h1{font-size:29px}.pbt-scanner{min-height:175px}.pbt-monitor{grid-template-columns:1fr}.pbt-info{grid-template-columns:1fr 1fr}.pbt-nav{position:fixed;z-index:3;right:0;bottom:0;left:0;display:flex;justify-content:space-around;padding:10px 6px calc(12px + env(safe-area-inset-bottom));border-top:1px solid #8e56dc66;border-radius:20px 20px 0 0;background:#07061bed;backdrop-filter:blur(18px)}.pbt-nav span{display:grid;justify-items:center;gap:4px;color:#d0c8e5;font-size:9px}.pbt-nav b{display:grid;place-items:center;width:23px;height:23px;border:1px solid currentColor;border-radius:7px}.pbt-nav .on{color:#fff;text-shadow:0 0 12px #cc40ff}.pbt-nav .on b{background:#882dff66;box-shadow:0 0 14px #b33dff} }
  `;
  ...
  const card = bot => `<article class="pbt-card" data-bot="${escapeHtml(bot.slug)}"><div class="pbt-icon">${escapeHtml((bot.kind === 'ai' ? 'AI' : bot.category || 'BT').slice(0, 2))}</div><div><span class="pbt-verified">VERIFIED</span><h2>${escapeHtml(bot.title)}</h2><p>${escapeHtml(bot.description)}</p></div><div class="pbt-tags">${tags(bot)}</div></article>`;
  const bars = Array.from({ length: 20 }, (_, index) => `<i style="height:${22 + ((index * 17) % 66)}%"></i>`).join('');

  const shell = () => `<section class="pbt"><header class="pbt-shell pbt-header"><div class="pbt-brand"><i>P</i>Profitera Bot</div><span class="pbt-status">Connected to Deriv</span><button class="pbt-close" data-close>Open builder</button></header><main class="pbt-shell pbt-main" data-page></main><nav class="pbt-nav"><span><b>H</b>Home</span><span><b>T</b>Trade</span><span><b>A</b>AI</span><span class="on"><b>B</b>Bots</span><span><b>R</b>Reports</span></nav></section>`;
  ...
  const mount = async () => {
    ...
    const page = document.querySelector('.pbt [data-page]');
