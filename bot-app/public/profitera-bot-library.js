(() => {
  ...
  const mount = async () => {
    ...
    try {
      ...
      grid.innerHTML = bots.map(bot => `<article class="profitera-bot-card" data-slug="${escapeHtml(bot.slug)}"><span class="profitera-bot-card__kind">${bot.kind === 'ai' ? 'AI-assisted' : 'Deriv bot'}</span>${bot.cover_image ? `<img class="profitera-library__image" src="${escapeHtml(bot.cover_image)}" alt="">` : ''}<h2>${escapeHtml(bot.title)}</h2><p>${escapeHtml(bot.description)}</p>${bot.ai_summary ? `<p style="margin-top:10px"><b>AI:</b> ${escapeHtml(bot.ai_summary)}</p>` : ''}<div class="profitera-bot-card__meta">${bot.market ? `<span>${escapeHtml(bot.market)}</span>` : ''}${bot.risk_level ? `<span>${escapeHtml(bot.risk_level)} risk</span>` : ''}${bot.minimum_stake ? `<span>Min $${escapeHtml(bot.minimum_stake)}</span>` : ''}${(bot.tags || []).map(tag => `<span>${escapeHtml(tag)}</span>`).join('')}</div><button ${bot.has_strategy ? '' : 'disabled'}>${bot.has_strategy ? 'Load in Bot Builder' : 'Analysis bot — setup required'}</button></article>`).join('');
