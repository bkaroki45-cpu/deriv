(() => {
  ...
  const publishBalanceFromMessage = event => { try { const m = JSON.parse(event.data), b = m.balance?.balance ?? m.buy?.balance_after ?? m.sell?.balance_after; if (b === undefined) return; const live = { balance: b, currency: m.balance?.currency || m.buy?.currency || m.sell?.currency || 'USD', account_id: localStorage.getItem('active_loginid') || '', at: Date.now() }; localStorage.setItem('profitera:live-balance', JSON.stringify(live)); fetch('/api/dashboard/live-balance/', { method: 'POST', credentials: 'same-origin', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(live) }).catch(() => {}); } catch (_) {} }; const onmessageDescriptor = Object.getOwnPropertyDescriptor(WebSocket.prototype, 'onmessage'); if (onmessageDescriptor?.set) Object.defineProperty(WebSocket.prototype, 'onmessage', { configurable: true, get: onmessageDescriptor.get, set(handler) { onmessageDescriptor.set.call(this, function(event) { publishBalanceFromMessage(event); return handler?.call(this, event); }); } });
