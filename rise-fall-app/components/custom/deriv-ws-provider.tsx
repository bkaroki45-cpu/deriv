...
function TradeResultNotification({ ws, isConnected, balance, currency }: { ws: DerivWS | null; isConnected: boolean; balance?: string | number; currency?: string }) {
  const [notice, setNotice] = useState<{ profit: number; currency: string; balance: string | number; id: string } | null>(null); const seen = useRef(new Map<string, boolean>());
  useEffect(() => { if (!ws || !isConnected) return; return ws.onMessage(data => { if (data.msg_type !== 'proposal_open_contract') return; const contract = data.proposal_open_contract as Record<string, unknown> | undefined; if (!contract?.contract_id) return; const id=String(contract.contract_id), closed=!!contract.is_sold||!!contract.is_expired||contract.status!=='open', wasOpen=seen.current.get(id); seen.current.set(id,!closed); if(wasOpen&&closed)setNotice({id,profit:Number(contract.profit||0),currency:String(contract.currency||currency||''),balance:String(contract.balance_after||balance||'')}); }); }, [ws,isConnected,balance,currency]);
  useEffect(()=>{if(!notice)return;const t=window.setTimeout(()=>setNotice(null),3500), k=(e:KeyboardEvent)=>e.key==='Escape'&&setNotice(null);window.addEventListener('keydown',k);return()=>{clearTimeout(t);window.removeEventListener('keydown',k)}},[notice]); if(!notice)return null; const won=notice.profit>0;
  return <aside role="status" aria-live="polite" style={{position:'fixed',zIndex:100,top:20,right:20,width:'min(390px, calc(100vw - 40px))',padding:'18px 42px 18px 78px',borderRadius:14,color:'#fff',background:won?'rgba(4,61,39,.96)':'rgba(72,17,30,.96)',border:`1px solid ${won?'#19d475':'#ff4056'}`,boxShadow:'0 20px 55px rgba(0,0,0,.42)',animation:'profitera-result-in .28s ease-out'}}><style>{'@keyframes profitera-result-in{from{opacity:0;transform:translateY(-20px)}to{opacity:1;transform:translateY(0)}}'}</style><span style={{position:'absolute',left:18,top:18,width:46,height:46,borderRadius:'50%',display:'grid',placeItems:'center',fontSize:22,background:won?'#17c96d':'#ff3d4d'}}>{won?'↗':'↘'}</span><button onClick={()=>setNotice(null)} aria-label="Close trade result" style={{position:'absolute',right:10,top:7,border:0,background:'transparent',color:'#fff',fontSize:22,cursor:'pointer'}}>×</button><strong style={{fontSize:17}}>{won?'Trade Won! 🎉':'Trade Lost 😔'}</strong><div style={{marginTop:5,fontWeight:700}}>{won?'You gained ':'You lost '}{Math.abs(notice.profit).toFixed(2)} {notice.currency}</div><div style={{marginTop:5,color:'#e1e9ef'}}>New balance: {notice.balance} {notice.currency}</div></aside>;
}
